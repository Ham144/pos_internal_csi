...
//digunakan untuk memilih multi inventory sku
const PickMultiInventoriesDialog = ({
  inventoryList,
  tempSkuTerhubung,
  ...
  return (
    <dialog id="pickblokirsku" className="modal  ">
      <div className="relative  bg-white p-4 rounded min-w-[800px]">
        <h3 className="font-bold text-lg mb-7">Blokir Sku Untuk Diskon Ini</h3>
            ...
            <tbody className="text-sm overflow-y-auto text-center ">
              {inventoryList?.map((item) => (
                <tr
                  key={item._id}
                  className={`${
                    tempSkuTerhubung?.includes(item?.sku) && "bg-green-400"
                  }`}
                >
                  <td>{item?.sku}</td>
                  ...
                  <td
                    onClick={() => {
                      if (tempSkuTerhubung.includes(item.sku)) {
                        setTempSkuTerblokir((prev) =>
                          prev.filter((i) => i !== item.sku)
                        );
                        setTempInventoryBukaBlokir((prev) => [
                          ...prev,
                          item.sku,
                        ]);
                      } else {
                        ...
                        setTempSkuTerblokir((prev) => [...prev, item.sku]);
                      }
                    }}
                  >
        ...
        <div className="absolute bottom-0 left-0 right-0 flex justify-between px-4 py-2 bg-white border-t">
          <button
            ...
            onClick={() => {
              setTempSkuTerblokir([]);
              document.getElementById("pickblokirsku").close();
            }}
          >
            Batal
          </button>
          <button className="btn" onClick={handleKonfirmasiTerhubung}>
            Konfirmasi
          </button>
        </div>
      </div>
...
export default PickMultiInventoriesDialog;
